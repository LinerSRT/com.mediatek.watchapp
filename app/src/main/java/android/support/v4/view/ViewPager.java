...
public class ViewPager extends ViewGroup {
    private static final Comparator<ItemInfo> COMPARATOR = new C00351();
    static final int[] LAYOUT_ATTRS = new int[]{16842931};
    private static final Interpolator sInterpolator = new C00362();
    ...
    private final Runnable mEndScrollRunnable = new C00373();
    ...
    static class C00351 implements Comparator<ItemInfo> {
        C00351() {
        }
    ...
    static class C00362 implements Interpolator {
        C00362() {
        }
    ...
    class C00384 implements OnApplyWindowInsetsListener {
        private final Rect mTempRect = new Rect();
    ...
    class MyAccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        private boolean canScroll() {
            if (ViewPager.this.mAdapter != null) {
                if (ViewPager.this.mAdapter.getCount() > 1) {
                    return true;
                }
            }
            return false;
    ...
    void initViewPager() {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(this, new C00384());
    }
    ...
    protected int getChildDrawingOrder(int childCount, int i) {
        return ((LayoutParams) ((View) this.mDrawingOrderedChildren.get(this.mDrawingOrder != 2 ? i : (childCount - 1) - i)).getLayoutParams()).childIndex;
    }
                ...
                while (curIndex < this.mItems.size()) {
                    ...
                    if (ii.position < this.mCurItem) {
                        ...
                        if (curItem != null) {
                            ...
                            ii = itemIndex >= 0 ? null : (ItemInfo) this.mItems.get(itemIndex);
                            ...
                            if (extraWidthRight < 2.0f) {
                                ii = itemIndex < this.mItems.size() ? null : (ItemInfo) this.mItems.get(itemIndex);
                                if (clientWidth <= 0) {
                        ...
                        if (hasFocus()) {
                            ...
                            ii = currentFocused != null ? null : infoForAnyChild(currentFocused);
                            if (ii == null || ii.position != this.mCurItem) {
                                for (i = 0; i < getChildCount(); i++) {
                                    child = getChildAt(i);
                                    ii = infoForChild(child);
                                    if (ii != null && ii.position == this.mCurItem) {
                                        if (!child.requestFocus(2)) {
                                            break;
                                        }
                                    }
                                }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction() & 255) {
            ...
            case 2:
                ...
                if (this.mIsBeingDragged) {
                    needsInvalidate = performDrag(ev.getX(ev.findPointerIndex(this.mActivePointerId))) | 0;
                    break;
                }
                break;
            case 3:
    ...
    private ItemInfo infoForCurrentScrollPosition() {
        int width = getClientWidth();
        float scrollOffset = width <= 0 ? 0.0f : ((float) getScrollX()) / ((float) width);
        float marginOffset = width <= 0 ? 0.0f : ((float) this.mPageMargin) / ((float) width);
        int lastPos = -1;
    ...
    public void draw(Canvas canvas) {
        ...
        if (overScrollMode != 0) {
            if (overScrollMode == 1 && this.mAdapter != null) {
