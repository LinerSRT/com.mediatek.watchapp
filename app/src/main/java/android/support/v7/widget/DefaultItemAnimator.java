        ...
        if (removalsPending || movesPending || additionsPending || changesPending) {
            ...
            if (additionsPending) {
                ...
                if (removalsPending || movesPending || changesPending) {
                    ViewCompat.postOnAnimationDelayed(((ViewHolder) additions.get(0)).itemView, adder, (!removalsPending ? 0 : getRemoveDuration()) + Math.max(!movesPending ? 0 : getMoveDuration(), !changesPending ? 0 : getChangeDuration()));
                } else {
                    adder.run();
                }
            }
