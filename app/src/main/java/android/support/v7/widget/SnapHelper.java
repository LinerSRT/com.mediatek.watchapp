...
public abstract class SnapHelper extends OnFlingListener {
    ...
    private final OnScrollListener mScrollListener = new C00711();

    /* renamed from: android.support.v7.widget.SnapHelper$1 */
    class C00711 extends OnScrollListener {
    ...
    public boolean onFling(int velocityX, int velocityY) {
        ...
        if (Math.abs(velocityY) > minFlingVelocity || Math.abs(velocityX) > minFlingVelocity) {
            if (snapFromFling(layoutManager, velocityX, velocityY)) {
                z = true;
            }
        }
        return z;
    ...
    protected LinearSmoothScroller createSnapScroller(LayoutManager layoutManager) {
        return layoutManager instanceof ScrollVectorProvider ? new LinearSmoothScroller(this.mRecyclerView.getContext()) {
