...
public class DefaultConfigurationFactory {
    ...
    public static Executor createExecutor(int threadPoolSize, int threadPriority, QueueProcessingType tasksProcessingType) {
        boolean lifo = false;
        if (tasksProcessingType == QueueProcessingType.LIFO) {
            lifo = true;
        }
        return new ThreadPoolExecutor(threadPoolSize, threadPoolSize, 0, TimeUnit.MILLISECONDS, !lifo ? new LinkedBlockingQueue() : new LIFOLinkedBlockingDeque(), createThreadFactory(threadPriority, "uil-pool-"));
    }
    ...
    public static DiskCache createDiskCache(Context context, FileNameGenerator diskCacheFileNameGenerator, long diskCacheSize, int diskCacheFileCount) {
        ...
        if (obj != null || diskCacheFileCount > 0) {
            try {
