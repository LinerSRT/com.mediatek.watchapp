...
public class BaseImageDecoder implements ImageDecoder {
        ...
        if (imageStream != null) {
            try {
                ...
                if (decodedBitmap != null) {
                    ...
                    C0197L.m1e("Image can't be decoded [%s]", decodingInfo.getImageKey());
                }
            ...
            C0197L.m1e("No stream for image [%s]", decodingInfo.getImageKey());
            return null;
        }
    }
    ...
    protected com.nostra13.universalimageloader.core.decode.BaseImageDecoder.ExifInfo defineExifOrientation(java.lang.String r9) {
        ...
        r5 = com.nostra13.universalimageloader.core.download.ImageDownloader.Scheme.FILE;	 Catch:{ IOException -> 0x002e }
    ...
    L_0x002e:
        ...
        r7 = 0;
        r6[r7] = r9;
        com.nostra13.universalimageloader.utils.C0197L.m4w(r5, r6);
        goto L_0x0019;
        */
    ...
    protected Options prepareDecodingOptions(ImageSize imageSize, ImageDecodingInfo decodingInfo) {
        ...
        if (scale > 1 && this.loggingEnabled) {
            C0197L.m0d("Subsample original image (%1$s) to %2$s (scale = %3$d) [%4$s]", imageSize, imageSize.scaleDown(scale), Integer.valueOf(scale), decodingInfo.getImageKey());
        }
    ...
    protected Bitmap considerExactScaleAndOrientatiton(Bitmap subsampledBitmap, ImageDecodingInfo decodingInfo, int rotation, boolean flipHorizontal) {
        ...
        if (scaleType == ImageScaleType.EXACTLY || scaleType == ImageScaleType.EXACTLY_STRETCHED) {
            ...
            ImageSize targetSize = decodingInfo.getTargetSize();
            ViewScaleType viewScaleType = decodingInfo.getViewScaleType();
            if (scaleType != ImageScaleType.EXACTLY_STRETCHED) {
                z = false;
            } else {
                z = true;
            }
            float scale = ImageSizeUtils.computeImageScale(srcSize, targetSize, viewScaleType, z);
            if (Float.compare(scale, 1.0f) != 0) {
                m.setScale(scale, scale);
                if (this.loggingEnabled) {
                    C0197L.m0d("Scale subsampled image (%1$s) to %2$s (scale = %3$.5f) [%4$s]", srcSize, srcSize.scale(scale), Float.valueOf(scale), decodingInfo.getImageKey());
                }
            }
        }
        if (flipHorizontal) {
            ...
            if (this.loggingEnabled) {
                C0197L.m0d("Flip image horizontally [%s]", decodingInfo.getImageKey());
            }
        }
        if (rotation != 0) {
            m.postRotate((float) rotation);
            if (this.loggingEnabled) {
                C0197L.m0d("Rotate image on %1$d° [%2$s]", Integer.valueOf(rotation), decodingInfo.getImageKey());
            }
        }
