...
public class AppListCustomUtil {
    ...
    public static ArrayList<AppInfo> getAppList(Context context) {
        ...
        applist = reorderApplist(applist);
        Log.i("guoxiaolong", "applist116 =" + applist);
        return applist;
    }
    ...
    public static Drawable ReplaceAppIcon(Context context, String cls) {
        cls = cls.toLowerCase().replace(".", "_").replace("$", "_");
        Log.v("caojinliang", "AppListCustomUtil::icon_id=" + cls);
        int iconID = context.getResources().getIdentifier(cls.toString(), "mipmap", "com.mediatek.watchapp");
    ...
    public static Drawable IndexUpdate(Drawable icon, Context context, String cls) {
        cls = cls.toLowerCase().replace(".", "_").replace("$", "_");
        if (cls.equals("com_android_mms_ui_bootactivity")) {
            return new BitmapDrawable(genertCountsOnBitmap(context, drawableToBitmap(icon), 1));
        }
        if (cls.equals("com_android_dialer_dialtactswearactivity")) {
